(function() {
            ...
            if (menuAccess) {
                ...
                buttonControl = false;
                    ...
                    '<div class="menu">' +
                        ...
                        '</div>' +
                        '<div class="listMenu">' +
                            '<div class="menuBlock resume">Resume</div>' +
                            '<div class="menuBlock newGameButton">New game</div>' +
                            '<div class="menuBlock backToHub">Back to Hub</div>' + // Moved to the bottom
                        '</div>' +
                    '</div>';
                ...
                var backToHubButton = document.querySelector(
                    ".listMenu div:nth-child(3)"
                );
                backToHubButton.onclick = function () {
                    window.location.href = "hub_game.html"; // Redirect to the game hub
                };
    ...
    // Add touch event listeners to the display area
    const displayElement = document.getElementById("display");
    
    displayElement.addEventListener("touchstart", function (event) {
        const touch = event.touches[0];
        touchStartX = touch.clientX;
        touchStartY = touch.clientY;
    });
    
    displayElement.addEventListener("touchmove", function (event) {
        const touch = event.touches[0];
    ...
    displayElement.addEventListener("touchend", function () {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY)) {
            ...
            if (deltaY > 30) {
                ...
                // Swipe Up (optional, can be used for rotation)
                objFigure.rotateFigure();
            }
        }
    ...
    // Optional: Single tap to rotate the piece
    displayElement.addEventListener("touchstart", function (event) {
